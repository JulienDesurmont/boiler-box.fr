...
class checkLoginListener {
    ...
    public function __construct(Router $router, detectLogin $detectLogin, Session $session, SecurityContext $security, $serviceLog) {
    ...
    public function checkLogin(GetResponseEvent $event) {
		...
		if ($this->security->getToken()) {
			...
			if (gettype($user) != "string") {
				...
				if (($event->getRequest()->get('_route') != 'fos_user_security_login') && ($event->getRequest()->get('_route') != 'lci_boilerbox_accesSite') && ($event->getRequest()->get('_route') != '')) {
