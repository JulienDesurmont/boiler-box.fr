...
public function afficheModuleProblemesAction() {
	$this->constructPerso();
	...
	$this->getRequest()->getSession()->set('ObjetRecherche', $objet_recherche_probleme);
	$this->getRequest()->getSession()->set('fromVar', 'parcDesModules');
...
public function afficheEquipementProblemesAction() {
    $this->constructPerso();
    $objet_recherche_probleme = new ObjRechercheProbleme();
    $objet_recherche_probleme->setEquipementId($_POST['equipement_id']);
    $objet_recherche_probleme->setNonCloture(true);
    $this->getRequest()->getSession()->set('ObjetRecherche', $objet_recherche_probleme);
	$this->getRequest()->getSession()->set('fromVar', 'parcDesEquipements');
    // Recherche en fonction des paramètres de l'objet
    $tableau_probleme_technique = $this->getDoctrine()->getManager()->getRepository('LciBoilerBoxBundle:ProblemeTechnique')->myFindProblemes($objet_recherche_probleme, 'entities', 'dateSignalement', 'DESC');
    return $this->render('LciBoilerBoxBundle:GestionParc:liste_problemes.html.twig', array(
        'tableau_problemes' => $tableau_probleme_technique
...
public function rechercheProblemeAction() {
	$this->constructPerso();
	$objet_recherche_probleme = new ObjRechercheProbleme();
	$formulaire = $this->createForm(new ObjRechercheProblemeType(), $objet_recherche_probleme);
	$requete = $this->get('request');
	if ($requete->getMethod() == 'POST') {
		$formulaire->handleRequest($requete);
		if ($formulaire->isValid()) {
			if (isset($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['chk_intervenant'])) {
				$objet_recherche_probleme->setIntervenantId($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['intervenant']);
			}
            if (isset($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['chk_module'])) {
                $objet_recherche_probleme->setModuleId($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['module']);
            }
            if (isset($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['chk_equipement'])) {
                $objet_recherche_probleme->setEquipementId($_POST['lci_boilerboxbundle_ObjRechercheProbleme']['equipement']);
			}
			$this->getRequest()->getSession()->set('ObjetRecherche', $objet_recherche_probleme);
			// Recherche en fonction des paramètres de l'objet
			$tableau_probleme_technique = $this->getDoctrine()->getManager()->getRepository('LciBoilerBoxBundle:ProblemeTechnique')->myFindProblemes($objet_recherche_probleme, 'entities', 'dateSignalement', 'DESC');
            ...
            $requete->getSession()->getFlashBag()->add('info', 'Formulaire invalide : '.$formulaire->getErrorsAsString());
        }
	}
...
public function afficheListeProblemesAction(){
	$this->constructPerso();
	$objet_recherche_probleme = new ObjRechercheProbleme();
	$objet_recherche_probleme->setNonCloture(true);
	$this->getRequest()->getSession()->set('ObjetRecherche', $objet_recherche_probleme);
...
public function triRechercheProblemeAction() {
	$this->constructPerso();
	$old_champs_tri = $this->getRequest()->getSession()->get('ChampsTri', 'dateSignalement');
	$objet_recherche_probleme = $this->getRequest()->getSession()->get('ObjetRecherche', array());
	$ordre_tri = $this->getRequest()->getSession()->get('OrdreTri', 'DESC');
...
public function exportListeProblemesAction(){
	$this->constructPerso();
	$objet_recherche_probleme = $this->getRequest()->getSession()->get('ObjetRecherche', array());
	$champs_tri = $this->getRequest()->getSession()->get('ChampsTri', 'dateSignalement');
	$ordre_tri = $this->getRequest()->getSession()->get('OrdreTri', 'DESC');
	$tableau_probleme_technique = $this->getDoctrine()->getManager()->getRepository('LciBoilerBoxBundle:ProblemeTechnique')->myFindProblemes($objet_recherche_probleme, 'entities', $champs_tri, $ordre_tri);
...
public function problemeTechniqueAffichageAction() {
	$this->constructPerso();
	...
	if ($requete->getMethod() == 'POST'){
		...
		if ($form_probleme_technique->isValid()){
			...
			$requete->getSession()->getFlashBag()->add('info', 'Problème technique mis à jour');
		} else {
...
public function changeEquipementsAction(){
    ...
    if (isset($_POST['id_equipement'])) {
        ...
        $requete->getSession()->getFlashBag()->add('info', 'Equipement non renseigné');
    ...
    if ($requete->getMethod() == 'POST') {
        ...
        if ($form_equipement->isSubmitted()) {
            if ($form_equipement->isValid()) {
                $requete->getSession()->getFlashBag()->add('info', 'Enregistrement effectué.');
...
public function triParcModulesAction(){
    $this->constructPerso();
    $old_champs_tri = $this->getRequest()->getSession()->get('ChampsTriParcModules', 'numero');
    $ordre_tri = $this->getRequest()->getSession()->get('OrdreTriParcModules', 'DESC');
    if (isset($_POST['champs_tri'])) {
        $champs_tri = $_POST['champs_tri'];
    } else {
        $champs_tri = $old_champs_tri;
    }
    $this->getRequest()->getSession()->set('ChampsTriParcModules', $champs_tri);
    // Si un tri sur une nouvelle colonne est demandé : Tri DESC
    if ($champs_tri != $old_champs_tri) {
        $this->getRequest()->getSession()->set('OrdreTriParcModules', 'ASC');
        $ordre_tri = 'ASC';
    } else {
        // Si une tri sur la même colonne est demandé, on inverse le sens du tri
        if ($ordre_tri == 'ASC') {
            $this->getRequest()->getSession()->set('OrdreTriParcModules', 'DESC');
            $ordre_tri = 'DESC';
        } else {
            $this->getRequest()->getSession()->set('OrdreTriParcModules', 'ASC');
            $ordre_tri = 'ASC';
...
public function triParcEquipementsAction(){
    $this->constructPerso();
    $old_champs_tri = $this->getRequest()->getSession()->get('ChampsTriParcEquipements', 'numero');
    $ordre_tri = $this->getRequest()->getSession()->get('OrdreTriParcEquipements', 'DESC');
    if (isset($_POST['champs_tri'])) {
        $champs_tri = $_POST['champs_tri'];
    } else {
        $champs_tri = $old_champs_tri;
    }
    $this->getRequest()->getSession()->set('ChampsTriParcEquipements', $champs_tri);
    // Si un tri sur une nouvelle colonne est demandé : Tri DESC
    if ($champs_tri != $old_champs_tri) {
        $this->getRequest()->getSession()->set('OrdreTriParcEquipements', 'ASC');
        $ordre_tri = 'ASC';
    } else {
        // Si une tri sur la même colonne est demandé, on inverse le sens du tri
        if ($ordre_tri == 'ASC') {
            $this->getRequest()->getSession()->set('OrdreTriParcEquipements', 'DESC');
            $ordre_tri = 'DESC';
        } else {
            $this->getRequest()->getSession()->set('OrdreTriParcEquipements', 'ASC');
            $ordre_tri = 'ASC';
...
public function changeModulesAction(){
	...
	} elseif (isset($_POST['lci_boilerboxbundle_module']['id'])) {
		$idModule = $_POST['lci_boilerboxbundle_module']['id'];
	} else {
		$requete->getSession()->getFlashBag()->add('info', 'Module non renseigné');
				...
				if ($entity_module->getPresent() != $old_presence) {
					...
					if ($entity_module->getPresent() == false){
						...
						if ($nombre_problemes_bloquants != 0){
							$requete->getSession()->getFlashBag()->add('info', " - AVERTISSEMENT: Le module ".$entity_module->getNumero()." (".$entity_module->getNom().") a quitté le parc avec des problèmes bloquants non clos !");
						}
					} else {
